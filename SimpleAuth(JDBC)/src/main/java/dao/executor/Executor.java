...
public class Executor {
    ...
    public static <T> T execQuery(Connection connection, Map.Entry<String, String[]> query, Function<ResultSet, T> resultsFunc) throws SQLException {
        try (PreparedStatement stmt = connection.prepareStatement(query.getKey())){
            ...
            for (int i = 0; i < query.getValue().length; i++) {
                stmt.setString(i + 1, query.getValue()[i]);
            }
    ...
    public static Boolean execQuery(Connection connection, Map.Entry<String, String[]> query) throws SQLException {
