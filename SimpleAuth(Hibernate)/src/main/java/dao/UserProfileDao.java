...
public class UserProfileDao implements Dao<UserProfile> {
    ...
    public UserProfile getByLogin(String login) {
        try (Session session = sessionFactory.openSession()) {
            Query<UserProfile> query = session.createQuery("FROM UserProfile U WHERE U.login = :login");
            query.setParameter("login", login);
            List<UserProfile> upList= query.list();
            ...
            System.out.println(String.format("Failed to get user by login (%s).", e.getMessage()));
            return null;
        }
    }
    ...
    public List<UserProfile> getAll() {
        try (Session session = sessionFactory.openSession()) {
            Query<UserProfile> query = session.createQuery("FROM UserProfile");
            return query.list();
            // // Get criteria fabric
            // CriteriaBuilder builder = session.getCriteriaBuilder();
            // // Get query by class (need to get this type)
            ...
            System.out.println(String.format("Failed to get all users (%s).", e.getMessage()));
